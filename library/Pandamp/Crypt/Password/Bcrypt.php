 ...
 * @package    Zend_Crypt
 */
class Pandamp_Crypt_Password_Bcrypt implements Pandamp_Crypt_Password_PasswordInterface
{
     ...
     *
     * @param array $options
     * @throws Pandamp_Crypt_Exception
     */
    public function __construct($options = array())
    {
        if (!empty($options)) {
            if (!is_array($options)) {
                throw new Pandamp_Crypt_Exception(
    ...
    {
        if (empty($this->salt)) {
            $salt = Pandamp_Math_Rand::getBytes(self::MIN_SALT_SIZE);
        } else {
            $salt = $this->salt;
        }
        ...
        if (strlen($hash) <= 13) {
            throw new Pandamp_Crypt_Exception('Error during the bcrypt generation');
        }
        return $hash;
    }
